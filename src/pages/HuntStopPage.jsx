...
import { ArrowLeft, ArrowRight, CheckCircle2, Compass, Home, Lock, MapPinned, Route, Sparkles, Ticket } from 'lucide-react'
import { getRouteBySlug, getStop } from '../data/huntData'
import { getFirstAvailableStop, isStopComplete, isStopUnlocked, markStopComplete, verifyStopAnswer } from '../lib/huntGate'
...
export default function HuntStopPage() {
  ...
  const [complete, setComplete] = useState(() => category && stopId ? isStopComplete(category, stopId) : false)
  const [proofValue, setProofValue] = useState('')
  const [proofError, setProofError] = useState('')

  useEffect(() => {
    if (category && stopId) setComplete(isStopComplete(category, stopId))
  }, [category, stopId])
        ...
        <section className="rounded-[2rem] border border-[#e3a7a5]/18 bg-black/20 p-6 sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-7 text-[#f7f1e8]/84">{stop.clue}</p>
            </div>
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
            <div className="space-y-4">
              ...
              {stop.validationMode === 'answer' ? (
                <div className="rounded-[1.5rem] border border-[#e3a7a5]/18 bg-black/20 p-5 sm:p-6">
                  ...
                  <div className="mt-5"><button type="button" onClick={onMarkComplete} className="inline-flex min-h-12 items-center rounded-full bg-[#e3a7a5] px-5 py-3 text-sm font-black uppercase tracking-[0.14em] text-[#264636] transition hover:bg-[#efbbb9]">submit answer</button></div>
                </div>
              ) : (
                <div className="rounded-[1.5rem] border border-[#e3a7a5]/18 bg-black/20 p-5 sm:p-6">
                  <div className="flex items-center gap-3 text-[#e3a7a5]"><Ticket className="h-5 w-5" /><h2 className="text-lg font-black uppercase tracking-tight">{stop.validationMode === 'volunteer' ? 'volunteer checkpoint' : 'onsite completion'}</h2></div>
                  <p className="mt-4 text-sm leading-7 text-[#f7f1e8]/78">{stop.validationMode === 'volunteer' ? 'This stop needs a real world check in or volunteer handoff.' : 'This stop is completed in the world. Mark it complete once you have actually done it.'}</p>
                  <div className="mt-5"><button type="button" onClick={onMarkComplete} className="inline-flex min-h-12 items-center rounded-full bg-[#e3a7a5] px-5 py-3 text-sm font-black uppercase tracking-[0.14em] text-[#264636] transition hover:bg-[#efbbb9]">mark complete</button></div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              <div className="rounded-[1.5rem] border border-[#e3a7a5]/18 bg-black/20 p-5 sm:p-6">
                ...
                <p className="mt-4 text-sm leading-7 text-[#f7f1e8]/62">QRs are print-only in this build and should be posted in the world, not shown on this page.</p>
              </div>
              ...
              <div className="rounded-[1.5rem] border border-[#e3a7a5]/18 bg-black/20 p-5 sm:p-6">
                ...
                <p className="mt-4 text-sm leading-7 text-[#f7f1e8]/78">Complete the current stop, then move forward. Volunteers can still hand out tickets or physical clues where needed.</p>
                <div className="mt-5 flex flex-wrap gap-3">
                  {nextStop ? <Link to={`/hunt/${category}/${nextStop.id}`} className="inline-flex min-h-12 items-center rounded-full border border-[#e3a7a5]/18 bg-black/20 px-5 py-3 text-sm font-black uppercase tracking-[0.14em] text-[#f7f1e8] transition hover:bg-[#e3a7a5]/10">next stop<ArrowRight className="ml-2 h-4 w-4" /></Link> : <Link to="/hunt" className="inline-flex min-h-12 items-center rounded-full border border-[#e3a7a5]/18 bg-black/20 px-5 py-3 text-sm font-black uppercase tracking-[0.14em] text-[#f7f1e8] transition hover:bg-[#e3a7a5]/10">route complete<CheckCircle2 className="ml-2 h-4 w-4" /></Link>}
                </div>
              </div>
            </div>
          </div>
