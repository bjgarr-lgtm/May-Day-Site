...
export default function ProgrammingView() {
  ...
  const rows = store.programming
    ...
    .filter((item) => {
      ...
      return (!query || hay.includes(query.toLowerCase())) && (statusFilter === "All" || item.status === statusFilter) && (categoryFilter === "All" || item.category === categoryFilter);
    });

  const handleSave = () => {
    if (!editor.activity.trim()) return window.alert("Activity name is required.");
  ...
  const syncPerformers = async () => {
    try {
      setSyncing(true);
      const count = await store.syncPerformersFromForms();
  ...
  const columns = [
    ...
    { key: "cost", label: "Cost", width: 100, render: (value) => value ? `$${Number(value).toFixed(0)}` : "—", exportValue: (value) => value || "" },
    { key: "status", label: "Status", width: 120, render: (value) => <span className={`ops-pill status-${slug(value)}`}>{value}</span>, exportValue: (value) => value },
    ...
    <div className="ops-page">
      <SectionCard title="Programming" subtitle="Activities, rooms, leads, needs, and cost.">
        <div className="ops-toolbar">
          ...
          <button type="button" className="ops-button ops-button-small ops-button-secondary" onClick={syncPerformers} disabled={syncing}>{syncing ? 'Syncing…' : 'Sync performers'}</button>
        ...
        <EditableTable tableKey="programming" rows={rows} onEdit={handleEdit} onDelete={(id) => store.removeItem("programming", id)} columns={columns} emptyLabel="No programming items yet." />
      </SectionCard>
