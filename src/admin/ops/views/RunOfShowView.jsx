...
export default function RunOfShowView() {
  ...
  const unassignedVolunteers = store.uncoveredVolunteerShifts;
  ...
  return (
    <div className="ops-page">
      ...
      <div className="ops-stat-grid ops-stat-grid-three">
        ...
        <div className="ops-stat-card"><div className="ops-stat-label">Volunteer issues</div><div className="ops-stat-value">{unassignedVolunteers.length + store.volunteerConflicts.length}</div></div>
      </div>
      <SectionCard title="Run of show" subtitle="Day-of control sheet.">
        <div className="ops-run-grid">
          ...
          <div className="ops-run-column"><h3>Volunteer shifts</h3><ul className="ops-list">{volunteerShifts.length ? volunteerShifts.map((item) => {
            const warnings = [];
            if (!item.name?.trim() || item.status === "Needs Assignment") warnings.push("uncovered");
            if (store.volunteerConflictsById?.[item.id]?.length) warnings.push("double-booked");
            return <li key={item.id}><strong>{item.role}</strong><span>{item.name || "Unassigned"}</span><span>{formatDateTime(item.shiftDate, item.shiftStart)}{item.shiftEnd ? ` to ${item.shiftEnd}` : ""}</span><span>{item.area}</span>{warnings.length ? <span>{warnings.join(" • ")}</span> : null}</li>;
          }) : <li className="ops-list-empty">Volunteer board is empty.</li>}</ul></div>
        </div>
      </SectionCard>
    </div>
  );
